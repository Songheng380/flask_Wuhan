...
def load_poi_data(sample: bool = SearchConfig.sample):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

    if sample:
        poi_path = os.path.join(BASE_DIR, "../../data/poi_sample.json")
    else:
        poi_path = os.path.join(BASE_DIR, "../../data/poi_full.json")
...
@api.route('/search')
def search_poi(POI_DATA, sample=SearchConfig.sample, FIELDS = SearchConfig.FIELDS):
    """
...
# # 矩形范围查询，还未实现，有误
@api.route('/bbox')
def bbox_query(POI_DATA, sample=SearchConfig.sample):
