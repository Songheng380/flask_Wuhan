...
function startRangeQuery() {
    ...
    rectangleTool.on('draw', function (event) {
        ...
        console.log("✅ 框选完成，坐标:", coords);

        // 框选完成后立即发请求，不会影响搜索按钮
        fetchBBoxData(coords);
    });
}

function fetchBBoxData(coords) {
    const url = `/api/search?min_lon=${coords.min_lon}&min_lat=${coords.min_lat}&max_lon=${coords.max_lon}&max_lat=${coords.max_lat}`;
    fetch(url)
        .then(res => res.json())
        .then(data => {
            clearMarkersOnly();  // 只清除旧标记，不清除矩形
            addMarkers(data);
            showResults(data, `框选区域查询结果`);
        })
        .catch(err => console.error("框选查询失败:", err));
}
...
window.clearMarkersOnly = clearMarkersOnly;
window.startRangeQuery = startRangeQuery;