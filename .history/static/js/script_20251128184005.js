...
function startRangeQuery() {
    ...
    rectangleTool.on('draw', function (event) {
        const rect = event.obj;       // AMap.Rectangle
        currentRectangle = rect;
        rectangleTool.close();        // 关闭绘制模式
        rectangleTool = null;
        ...
        // 只在这里发请求，不影响绘制确认
        fetch(`/search?min_lon=${coords.min_lon}&min_lat=${coords.min_lat}&max_lon=${coords.max_lon}&max_lat=${coords.max_lat}`)
            .catch(() => {});  // 失败也不处理
    });
}
