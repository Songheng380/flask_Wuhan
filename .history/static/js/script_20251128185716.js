...
function startRangeQuery() {
    ...
    rectangleTool.on('draw', function (event) {
        ...
        // 发请求
        const url = `/search?min_lon=${coords.min_lon}&min_lat=${coords.min_lat}&max_lon=${coords.max_lon}&max_lat=${coords.max_lat}&q=${encodeURIComponent(keyword)}`;
        fetch(url)
            .then(res => res.json())
            .then(data => {
                clearMarkers();
                addMarkers(data);
                showResults(data, keyword ? `"${keyword}" 的搜索结果` : '矩形范围内 POI');
            })
