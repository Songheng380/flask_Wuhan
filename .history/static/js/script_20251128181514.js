...
function startRangeQuery() {
    ...
    rectangleTool.on('draw', function (event) {
        // 获取绘制的矩形对象
        currentRectangle = event.obj;  // AMap.Rectangle
    ...
    const url = `/search?min_lon=${coords.min_lon}&min_lat=${coords.min_lat}&max_lon=${coords.max_lon}&max_lat=${coords.max_lat}`;

    //只发送请求，不处理返回
    fetch(url).catch(err => {
        console.warn("发送失败，但忽略:", err);
    });
}
